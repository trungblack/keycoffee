...
<form method="post" class="ui form" action="" enctype="multipart/form-data">
    ...
    <div class="ui segment">
        ...
        <a href="/gallery/admin" class="ui button">
        ...
        {% if model is defined and model.getId() %}
            <a href="/gallery/admin/delete/{{ model.getId() }}" class="ui button red">
    ...
    {% if model is defined %}
        ...
        <div class="ui segment attached work-with-image">
            ...
            <div class="ui cards gallery-item">
                {% for image in model.getRelated('GalleryImages', ['order': 'sortorder ASC']) %}
    ...
    $('.ui.form').form({
        title: {
            ...
            rules: [
                {
                    type: 'empty',
                    prompt : 'Specify the name of the gallery'
                }
            ]
        }
    }, {
...
<script>
    ...
    function saveGallery () {
        ...
        $.ajax({
            async: false,
            url: "/gallery/admin/saveGallery?lang={{  helper.constant('LANG') }}",
            data: {
                items: items,
                gallery: {% if model is defined %}{{ model.getId() }}{% else %}0{% endif %}
            },
            type: 'POST',
            dataType: "json",
